...
function fetchRepos(username){
    return fetch(`https://api.github.com/users/${username}/repos`).then(function(raw){
        if(!raw.ok){
...
function decorateProfile(details){
    let data = `<img src="${details.avatar_url}" alt="Profile Picture" class="w-24 h-24 rounded-full object-cover border-blue-500 border-2">
